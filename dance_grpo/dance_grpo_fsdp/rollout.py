...
class HFRollout:
    ...
    @staticmethod
    def _pad_or_truncate_conditioning(hidden_states: torch.Tensor,
    ...
    def _generate_minibatch(self, data: DataProto) -> DataProto:
        ...
        with patch("recipe.dance_grpo.mammothmoda2.utils.t2i_utils.FlowMatchEulerDiscreteScheduler", return_value=grpo_scheduler), \
             patch("recipe.dance_grpo.mammothmoda2.utils.t2i_utils.retrieve_timesteps", return_value=(grpo_scheduler.set_timesteps(num_inference_steps, device), num_inference_steps)), \
             patch("recipe.dance_grpo.mammothmoda2.utils.t2i_utils.encode_full_prompts", side_effect=_capturing_encode_full_prompts):
            with torch.no_grad(), torch.autocast(device_type=device, dtype=torch.bfloat16):
        ...
        all_latents = torch.stack(grpo_scheduler.all_latents, dim=1)      # [B, steps+1, C, H, W]
        all_log_probs = torch.stack(grpo_scheduler.all_log_probs, dim=1)  # [B, steps]
        sigma_schedule = grpo_scheduler.timesteps                          # [steps+1]
        ...
        text_hidden_states, text_attention_mask = self._pad_or_truncate_conditioning(
            text_hidden_states, text_attention_mask, conditioning_max_length,
        )
        ...
        if "negative_text_hidden_states" in captured_conditioning:
            ...
            negative_text_hidden_states, negative_text_attention_mask = self._pad_or_truncate_conditioning(
                negative_text_hidden_states, negative_text_attention_mask, conditioning_max_length,
            )
        ...
        if "image_hidden_states" in captured_conditioning:
            image_hidden_states = (
                captured_conditioning["image_hidden_states"].expand(rollout_n, -1, -1).contiguous()
            )
            image_attention_mask = (
                captured_conditioning["image_attention_mask"].expand(rollout_n, -1).contiguous()
            )
            ...
            image_hidden_states, image_attention_mask = self._pad_or_truncate_conditioning(
                image_hidden_states, image_attention_mask, conditioning_max_length,
            )
