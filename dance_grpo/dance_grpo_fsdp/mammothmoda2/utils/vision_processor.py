            ...
            if isinstance(message["content"], list):
                for ele in message["content"]:
                    if (
                        ...
                        or "image_url" in ele
                        or "video" in ele
                        or ele.get("type", "text") in ("image", "image_url", "image_gen", "video")
                    ):
...
def extract_ref_image(conversations: Union[List[Dict[str, Any]], List[List[Dict[str, Any]]]], max_pixels: int = 1024 * 1024) -> List[Image.Image]:
    ...
    for conversation in conversations:
        ...
        for message in conversation[::-1]:
            if isinstance(message["content"], list):
                for ele in message["content"][::-1]:
                    if "image_gen" in ele:
                        _ele = {
                            "image_gen": ele["image_gen"],
                            "max_pixels": max_pixels,
                        }
                        ref_image = fetch_image(_ele, image_patch_size=8)
                        break
...
def process_vision_info(
    ...
    return image_inputs, gen_image_inputs, video_inputs