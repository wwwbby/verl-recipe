...
class RMSNorm(nn.Module):
    def __init__(self, dim: int, eps: float = 1e-6):
        super().__init__()
        self.eps = eps
        self.weight = nn.Parameter(torch.ones(dim))  # gamma 参数
...
class ExtractEOS(nn.Module):
    def forward(self, tokens: torch.Tensor, eos_token_mask: torch.Tensor) -> torch.Tensor:
        B, _, D = tokens.shape
        eos_token_mask = torch.argmax(eos_token_mask, dim=-1)
...
class AttentionPoolingClassifier(nn.Module):
    def __init__(
        ...
        self.bn = nn.BatchNorm1d(dim, affine=False, eps=1e-6) if use_batch_norm else nn.Identity()
