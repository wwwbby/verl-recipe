...
class Mammothmoda2Qwen3VLVisionConfig(PretrainedConfig):
...
class Mammothmoda2Qwen3VLTextConfig(PretrainedConfig):
    ...
    ```python
    >>> from transformers import Qwen3VLTextModel, Mammothmoda2Qwen3VLTextConfig

    >>> # Initializing a Qwen3VL style configuration
    >>> configuration = Mammothmoda2Qwen3VLTextConfig()
...
class Mammothmoda2Qwen3VLConfig(PretrainedConfig):
    r"""
    ...
    Args:
        text_config (`Union[PreTrainedConfig, dict]`, *optional*, defaults to `Mammothmoda2Qwen3VLTextConfig`):
            The config object or dictionary of the text backbone.
        vision_config (`Union[PreTrainedConfig, dict]`,  *optional*, defaults to `Mammothmoda2Qwen3VLVisionConfig`):
    ...
    ```python
    >>> from transformers import Qwen3VLForConditionalGeneration, Mammothmoda2Qwen3VLConfig

    >>> # Initializing a Qwen3-VL style configuration
    >>> configuration = Mammothmoda2Qwen3VLConfig()
    ...
    model_type = "mammothmoda2_qwen3_vl"
    sub_configs = {"vision_config": Mammothmoda2Qwen3VLVisionConfig, "text_config": Mammothmoda2Qwen3VLTextConfig}
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if isinstance(text_config, dict):
            for key in text_extra_kwargs:
                if key not in text_config:
                    text_config[key] = text_extra_kwargs[key]
            self.text_config = self.sub_configs["text_config"](**text_config)
        elif text_config is None:
            # For BC use all kwargs to init `TextConfig`
            for key in text_extra_kwargs:
                if key not in kwargs:
                    kwargs[key] = text_extra_kwargs[key]
...
__all__ = ["Mammothmoda2Qwen3VLConfig", "Mammothmoda2Qwen3VLTextConfig"]
