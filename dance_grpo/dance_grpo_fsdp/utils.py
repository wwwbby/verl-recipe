...
from torch.distributed.fsdp import CPUOffload, FullStateDictConfig, MixedPrecision
...
def load_repeat_data(file_index, grpo_size, root_dir="./data"):
    ...
    negative_prompt_attention_mask = torch.load(
        os.path.join(root_dir, "negative_prompt_attention_mask", file_index)
    ).to("npu")

    def unqueeze_tensor(i):
        return i.unsqueeze(0) if i.shape[0] != 1 else i

    prompt_embed, prompt_attention_mask, negative_prompt_embed, negative_prompt_attention_mask = (
        unqueeze_tensor(prompt_embed),
        unqueeze_tensor(prompt_attention_mask),
        unqueeze_tensor(negative_prompt_embed),
        unqueeze_tensor(negative_prompt_attention_mask),
    )
...
def init_fsdp_module(module, strategy="fsdp2", process_group=None, only_forward=False):
    ...
    auto_wrap_policy = get_fsdp_wrap_policy(module=module, config=None, is_lora=False)
